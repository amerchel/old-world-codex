...
export default function InventoryClient({ data }: { data: InventorySection[] }) {
    const { activeImageIndex, setActiveImageIndex } = useImageCarousel(
        data.map(section => ({
                            ...
                            return (
                              ...
                              `}
                                    ...
                                    <div className="w-full max-w-full md:w-[55%] text-secondary flex flex-col justify-between h-full">
                                        <div>
                                            <h3 className="text-lg sm:text-xl font-semibold text-primary mb-2">{name}</h3>
                                            <p className="whitespace-pre-line text-sm sm:text-base mb-4">{description}</p>
                                        </div>
                                        {stats.length > 0 && (
                                            <ul className="text-sm sm:text-base mt-6 space-y-2 divide-y divide-white/30">
                                                {stats.map((stat, idx) => (
                                                    <li key={idx} className="pt-2 first:pt-0">{stat}</li>
                                                ))}
                                            </ul>
                                        )}
                                    </div>
